const {SolidCircleCollider, SolidPolygonCollider, SolidWallCollider, CircleCollider} = require('./collidersModule.js');
...
class CollisionReposition{
  ...
  static circle2circle(c1, c2){
    if(c2 instanceof SolidCircleCollider){
...
function checkForCollision(e1, e2){
  var collisions = [];
  if(e1 instanceof CircleCollider && e2 instanceof SolidCircleCollider){
  ...
  }else if(e1 instanceof CircleCollider && e2 instanceof SolidWallCollider){
    if(CollisionDetection.circle2wall(e1, e2))
      collisions.push(new Collision(e1, e2));
  }else if(e1 instanceof CircleCollider && e2 instanceof SolidPolygonCollider){
    for(let i = 0; i < e2.walls.length; i++){
      if(CollisionDetection.circle2wall(e1, e2.walls[i]))
...
function resolveCollision(_col){
  if(_col.e2 instanceof SolidWallCollider){
    CollisionReposition.circle2wall(_col.e1, _col.e2);
    CollisionResponse.circle2wall(_col.e1, _col.e2);
  }else{
