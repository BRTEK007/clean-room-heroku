...
function triggerCircle2Circle(player, bullet) {
  ...
  if (CirclesOverlap(bullet.transform.pos.x, bullet.transform.pos.y, bullet.radius, player.transform.pos.x, player.transform.pos.y, player.radius)) {
    ...
    let vx = bullet.rigidbody.vel.x;
    let vy = bullet.rigidbody.vel.y;
...
function dynamicCircleX2(player1, player2) {
  if (!CirclesOverlap(player1.transform.pos.x, player1.transform.pos.y, player1.radius, player2.transform.pos.x, player2.transform.pos.y, player2.radius)) return;
  var fDistance = Math.sqrt(Math.pow(player1.transform.pos.x - player2.transform.pos.x, 2) + Math.pow(player1.transform.pos.y - player2.transform.pos.y, 2));
  var fOverlap = (fDistance - player1.radius - player2.radius);
  player1.transform.pos.x -= fOverlap * (player1.transform.pos.x - player2.transform.pos.x) / fDistance;
  player1.transform.pos.y -= fOverlap * (player1.transform.pos.y - player2.transform.pos.y) / fDistance;
  player2.transform.pos.x += fOverlap * (player1.transform.pos.x - player2.transform.pos.x) / fDistance;
  player2.transform.pos.y += fOverlap * (player1.transform.pos.y - player2.transform.pos.y) / fDistance;
  var nx = (player2.transform.pos.x - player1.transform.pos.x) / fDistance;
  var ny = (player2.transform.pos.y - player1.transform.pos.y) / fDistance;
  var tx = -ny;
  var ty = nx;
