...
function entity2entityCollision(e1, e2){
  if(!e1.isTrigger && !e2.isTrigger){
    ...
    if (CirclesOverlap(e1.transform.pos.x, e1.transform.pos.y, e1.radius, e2.transform.pos.x, e2.transform.pos.y, e2.radius)){
      pen_res_bb(e1, e2);
      //coll_res_bb(e1, e2);
    }
...
function pen_res_bb(b1, b2){
  let dist = b1.transform.pos.subtr(b2.transform.pos);
  let pen_depth = b1.radius + b2.radius - dist.mag();
  //dividing the penetration depth in the ratio of the inverse masses
  let pen_res = dist.unit().mult(pen_depth / (1/b1.rigidbody.mass + 1/b2.rigidbody.mass));
  b1.transform.pos = b1.transform.pos.add(pen_res.mult(1/b1.rigidbody.mass));
  b2.transform.pos = b2.transform.pos.add(pen_res.mult(-1/b1.rigidbody.mass));
}

function coll_res_bb(b1, b2){
  let normal = b1.transform.pos.subtr(b2.transform.pos).unit();
  let relVel = b1.rigidbody.vel.subtr(b2.rigidbody.vel);
  ...
  let impulse = vsep_diff / (1/b1.rigidbody.mass + 1/b2.rigidbody.mass);
  let impulseVec = normal.mult(impulse);

  b1.rigidbody.vel = b1.rigidbody.vel.add(impulseVec.mult(1/b1.rigidbody.mass));
  b2.rigidbody.vel = b2.rigidbody.vel.add(impulseVec.mult(-1/b2.rigidbody.mass));
}
