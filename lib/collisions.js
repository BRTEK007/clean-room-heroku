...
function checkForCollision(e1, e2){
  ...
  }else if(e1.shape instanceof CircleShape && e2 instanceof rectModule.SolidWallCollider){
    if(coll_det_circle2wall(e1, e2))
      return new Collision(e1, e2);
  }
...
function resolveCollision(_col){
  if(_col.e2 instanceof rectModule.SolidWallCollider){
    pen_res_circle2wall(_col.e1, _col.e2);
    coll_res_circle2wall(_col.e1, _col.e2);
  }else{
...
function coll_res_circle2circle(c1, c2){
  if(c2 instanceof SolidCollider){
...
function coll_det_circle2wall(b1, w1){
  let ballToClosest = closestPointBW(b1, w1).subtr(b1.transform.pos);
  if (ballToClosest.mag() <= b1.shape.radius){
      return true;
  }
}

function pen_res_circle2wall(b1, w1){
  let penVect = b1.transform.pos.subtr(closestPointBW(b1, w1));
  b1.transform.pos = b1.transform.pos.add(penVect.unit().mult(b1.shape.radius-penVect.mag()));
}

function coll_res_circle2wall(b1, w1){
  let normal = b1.transform.pos.subtr(closestPointBW(b1, w1)).unit();
