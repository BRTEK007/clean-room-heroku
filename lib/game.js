...
class Game {
  constructor(map) {
    ...
    //console.log(this.solidColliders[0].transform.pos);
    ...
    for(var i = 0; i < this.entities.length; i++){
      ...
      for (let j = i+1; j < this.entities.length; j++) {
        if(!this.entities[j].collider.isDisabled){ 
          closestCollision = colModule.checkForCollision(this.entities[i].collider, this.entities[j].collider);
        }
      }
      
      for (let j = 0; j < this.solidColliders.length; j++) {
        closestCollision = colModule.checkForCollision(this.entities[i].collider, this.solidColliders[j]);
      }
