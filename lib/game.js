  ...
  update(delta) {
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      for (let j = 0; j < this.players.length; j++) {
        ...
        colModule.resolvePlayersCollision(this.players[i], this.players[j]);
      }
      ...
      for (let j = 0; j < this.staticColliders.length; j++) {
          if(this.staticColliders[j] instanceof rectModule.RectCollider)
            colModule.resolvePlayerRectCollision(this.players[i], this.staticColliders[j]);
          else if(this.staticColliders[j] instanceof rectModule.CircleCollider)
            colModule.resolvePlayerBallCollision(this.players[i], this.staticColliders[j]);
      }
    }
    ...
    for (let i = 0; i < this.bullets.length; i++) {
      ...
      for (let j = 0; j < this.staticColliders.length; j++) {
          if(this.staticColliders[j] instanceof rectModule.RectCollider)
            colModule.resolveBulletRectCollision(this.bullets[i], this.staticColliders[j]);
          else if(this.staticColliders[j] instanceof rectModule.CircleCollider)
      ...
      for (let j = 0; j < this.players.length; j++) {
        ...
        if(colModule.resolvePlayerBulletCollision(this.players[j], this.bullets[i]));
        continue;
      }
