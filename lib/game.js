...
class Game {
  ...
  update(delta) {
    ...
    for(var i = 0; i < this.entities.length; i++){
      ...
      if(this.entities[i].isDisabled) continue loop1;

      this.entities[i].update(delta);

      if(this.entities[i].collider.isDisabled)  continue loop1;

      loop2:
      for (let j = 0; j < this.entities.length; j++) {
        if(j == i || this.entities[j].collider.isDisabled || this.entities[j].isDestroyed) continue loop2;
